...
def procesar_wide(input_path: str,
    ...
    COL_FECHA = auto_col(df.columns,
        keywords=['fecha entrevista','fecha_entrevista','fecha de entrevista','fechaentrevista','fecha interv'],
    ...
    for c in df.columns:
        nc = _norm_str(c)
        if any(k in nc for k in ['codigo del centro','centro de tratamiento','servicio de tratamiento','centro/ servicio']) \
                or (nc == 'centro'):
